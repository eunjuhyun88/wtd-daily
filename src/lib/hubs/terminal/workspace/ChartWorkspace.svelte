<script lang="ts">
  /**
   * ChartWorkspace — TradingView-style multi-chart grid with integrated
   * MarketActionBar and IndicatorQuickRail.
   *
   * W-0588: Replaces ChartGridLayout + ChartColumn as the single desktop
   * chart surface. Supports 6 layout presets (identical to TradingView
   * free tier):
  ...
  interface Props {
    ...
    /** MarketActionBar props (optional, passed from TradeMode in single-mode) */
  ...
  // Show MarketActionBar only when at least one signal prop is present
  const showActionBar = $derived(
    direction !== null || reason !== null || change24hPct !== null
  );
</script>

<div class="chart-workspace">
  <!-- ── MarketActionBar (conditional) ── -->
  {#if showActionBar}
    <MarketActionBar
      compact
      {direction}
...
<style>
  ...
  .layout-btn.active {
    ...
    border-color: color-mix(in srgb, var(--sc-accent, #3b82f6) 40%, transparent);
    background: color-mix(in srgb, var(--sc-accent, #3b82f6) 8%, transparent);
  }
