<script lang="ts">
  ...
  import WarRoomHeaderSection from './WarRoomHeaderSection.svelte';
  import WarRoomSignalFeed from './WarRoomSignalFeed.svelte';
  import WarRoomFooterSection from './WarRoomFooterSection.svelte';
  // C02 카드 → 채팅으로 통합됨 (VerdictCard + handleScanComplete)
  import type { TokenFilter, ScanTab, SignalDiff, ScanHighlight } from './types';
  import './warroom.css';
